...
稳定版保留已有的 Quick Shulker 界面、快速打开、容器操作、公共 API 和客户端可选的服务端备用路径。
...
两个文件均可在 [3.0.4 Release](https://github.com/water2004/quickshulker/releases/tag/3.0.4) 下载。
...
| 客户端 | 服务端 | 行为 |
| --- | --- | --- |
| 已安装 | 已安装 | 完整的快速打开界面、物品栏操作、增强收纳袋界面；双方支持时还可使用 4.0 潜影盒直接协议。 |
| 未安装 | 已安装 | 原版客户端仍可加入。服务端右键功能使用原版菜单，收纳袋使用分页的原版 `9 x 6` 容器，不注册自定义菜单类型；客户端快捷键和悬停操作不可用。 |
| 已安装 | 未安装 | 客户端不会假装协议可用。不受支持的 Quick Shulker 交互会交还给原版或其他模组处理。 |
...
安装模组的客户端继续使用可滚动的 64 槽增强收纳袋界面。服务端安装本分支后不再注册旧的自定义收纳袋 `MenuType`，因此原版客户端仍可连接，并使用服务端分页的原版 `GENERIC_9x6` 界面。
...
主 GameTest 锁定当前的快速打开与潜影盒事务行为；`legacy-gametest/` 会另外使用构建产物验证 3.0 公共行为契约。`main` 分支上的标签发布 Minecraft 26.2 文件，`26.1` 分支上的标签发布 Minecraft 26.1 文件。
